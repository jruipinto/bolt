...
export class AssistenciasApiService extends EntityApiAbstration {
  ...
  private simplify(arr: any[] | null): any[] | null {
    ...
    return arr.map((item) =>
      item.qty ? { id: item.id, qty: item.qty } : { id: item.id }
    );
  }
   ...
   */
  private fullyDetailedAssistencias$ = (
    assistenciasFromApi$: Observable<Assistencia[]>
  ) =>
    assistenciasFromApi$.pipe(
      concatMap((assistenciasFromApi: Assistencia[]) =>
        concat(
          ...assistenciasFromApi.map((assistenciaFromApi) =>
            this.usersAPI.get(assistenciaFromApi.cliente_user_id).pipe(
              ...
              map((assistencia) =>
                deserialize(assistencia, 'registo_cronologico')
              ),
              ...
              concatMap((assistencia) =>
                concat(
                  ...
                  toArray(),
                  map(
                    (registo_cronologico): Assistencia => ({
                      ...assistencia,
                      registo_cronologico,
                    })
                  )
              ...
              concatMap((assistencia) =>
                concat(
                  ...assistencia.registo_cronologico.map((evento) => {
                    if (evento.editor_user_id) {
                      // check because of backwards compatibility
              ...
              concatMap(
                (assistencia): Observable<Assistencia> => {
                  ...
                  const registoCronologico = clone(
                  ...
                  const lastEvento = registoFiltered
                    ? registoFiltered[registoFiltered.length - 1]
                    : null;
  ...
  /**
   * This Observable is a more performant version of
   * fullyDetailedAssistencias$ because of .find() method
   */
  private minimallyDetailedAssistencias$ = (
    assistenciasFromApi$: Observable<Assistencia[]>
  ) =>
    assistenciasFromApi$.pipe(
      concatMap((assistenciasFromApi: Assistencia[]) =>
        concat(
          ...assistenciasFromApi.map((assistenciaFromApi) =>
            this.usersAPI.get(assistenciaFromApi.cliente_user_id).pipe(
              ...
              // get assistencia.tecnico from api or...
              concatMap(
                (assistencia): Observable<Assistencia> => {
                  if (assistencia.tecnico_user_id) {
                    return this.usersAPI.get(assistencia.tecnico_user_id).pipe(
                  ...
                  // if (!assistencia.tecnico_user_id && assistencia.id < 3300) then from the last evento on assistencia.registo_cronologico (backwards compatibility)

                  return of(assistencia).pipe(
                    concatMap((assistencia) => {
                    ...
                    map((assistencia) => {
                      return deserialize(assistencia, 'registo_cronologico');
                    }),
  ...
  find(query?: any) {
    return this.minimallyDetailedAssistencias$(
      super.find({
        ...query,
        query: {
          ...query.query,
  ...
  get(id: number) {
    return this.fullyDetailedAssistencias$(super.get(id));
  }
  ...
  onCreated() {
    return this.fullyDetailedAssistencias$(super.onCreated());
  }
